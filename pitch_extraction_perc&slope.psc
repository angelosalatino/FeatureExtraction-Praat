  ...
  if n==1
	...
	if numberOfFiles > 0
		  for ifile to numberOfFiles
			...
			#CALCOLO PERCENTUALE FRAME VOICED
			nFrames=Get number of frames
			nVoicedFrame= Count voiced frames
			perc= (nVoicedFrame * 100)/nFrames
			path$="./data/Pitch_perc_&_slope/File_voiced_"+emo$+".txt"
			fileappend 'path$' 'perc''newline$'
			
			#ESTRAGGO SLOPE
			slope=Get mean absolute slope... Hertz
			path$="./data/Pitch_perc_&_slope/File_slope_"+emo$+".txt"
			fileappend 'path$' 'slope''newline$'
